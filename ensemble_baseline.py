"""Baseline ensemble eval: rare agg + recall_slice × 5 seeds (Phase X)."""
import os
import sys
import numpy as np
import torch
...
CKPT_TMPL = f'{PROJECT_ROOT}/results/fedavg_noniid_20c_20r_sr0.25_aggrare_lossrecall_slice_s{{seed}}/best_global_model.pth'
...
def main():
    print(f'[baseline_ens] Device: {DEVICE}')
    print(f'[baseline_ens] Seeds: {SEEDS}')
    ...
    out = f'{PROJECT_ROOT}/paper_figures/ensemble_baseline.txt'
    with open(out, 'w') as f:
        f.write(f'Ensemble (softmax avg) over baseline (rare agg + recall_slice) seeds {SEEDS}\n\n')
