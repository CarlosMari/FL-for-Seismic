"""
Federated Proximal (FedProx) Simulation on F3 Seismic Dataset
=============================================================
Same as FedAvg F3 but with proximal regularization mu/2 * ||w - w_global||^2.

Usage:
    python train_fedprox_f3.py --num_clients 3 --split noniid --mu 0.01
    python train_fedprox_f3.py --num_clients 20 --split iid --mu 0.01
"""
...
def local_train_fedprox(model, loader, criterion, device, local_epochs, lr,
                        weight_decay, global_params, mu):
    model.train()
    model.to(device)
    ...
    for epoch in range(local_epochs):
        for images, targets, _ in loader:
            ...
            prox = 0.0
            for p, gp in zip(model.parameters(), global_params_device):
                prox += (p - gp).norm(2) ** 2
...
# ── Main ────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="FedProx on F3")
    ...
    exp_name = f"fedprox_f3_mu{mu}_{split_mode}_{num_clients}c_{num_rounds}r"
    save_dir = os.path.join(PROJECT_ROOT, "results", exp_name)
    os.makedirs(save_dir, exist_ok=True)

    print("=" * 70)
    print(f"FedProx (mu={mu}) — F3 Netherlands | {split_mode.upper()} | {num_clients} clients")
    print("=" * 70)
    print(f"Split mode             : {split_mode}")
    print(f"Num clients            : {num_clients}")
    print(f"Mu (proximal coeff)    : {mu}")
    ...
    wandb.init(
        project="FL-Seismic",
        group=f"fedprox_f3_{split_mode}",
        name=exp_name,
        config={
            "algorithm": "FedProx",
            ...
            "optimizer": "AdamW",
            "loss": "FocalLoss + DiceLoss + ProximalTerm",
            "focal_gamma": 2.0,
            "model": "UNet (2D)",
    ...
    print("Loading F3 seismic cube from SEGY (this may take a moment)...")
    seismic = load_f3_segy(F3_SEGY)
    labels = np.load(F3_LABELS)
    print(f"  Cube shape: {seismic.shape} (inline, crossline, depth)")
    ...
    train_crosslines = list(range(0, n_train))
    ...
    print("\n" + "=" * 70)
    print(f"Starting FedProx (mu={mu}, {split_mode.upper()}, {num_clients} clients)")
    print("=" * 70 + "\n")
    ...
    for round_idx in range(num_rounds):
        ...
        for c in range(num_clients):
            ...
            updated_weights = local_train_fedprox(
                ...
                device=DEVICE, local_epochs=local_epochs, lr=lr,
                weight_decay=WEIGHT_DECAY, global_params=global_params, mu=mu,
            )
        ...
        miou, class_iou = evaluate_global(global_model, test_loader, test_labels, DEVICE)
        round_time = time.time() - round_start

        print(f"  mIoU: {miou:.4f}")
        print(f"  Per-class IoU: {['%.4f' % v for v in class_iou]}")
        ...
        for i in range(NUM_CLASSES):
            log_dict[f"per_class_iou/class_{i}"] = class_iou[i]
    ...
    print(f"\nRound | mIoU   | Per-Class IoU")
    print("-" * 70)
    for h in history:
        cls = " ".join([f"{v:.3f}" for v in h['per_class_iou']])
        print(f"  {h['round']:2d}  | {h['miou']:.4f} | [{cls}]")
