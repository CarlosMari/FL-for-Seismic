"""
Federated Averaging (FedAvg) Simulation on F3 Seismic Dataset
=============================================================
Same pipeline as the Parihaka experiment but on the F3 Netherlands dataset.
Since F3 has no pre-split test set, we hold out the last 20% of crosslines
as a global validation set and partition the rest across clients.

Usage:
    python train_federated_f3.py --num_clients 3 --split noniid
    python train_federated_f3.py --num_clients 20 --split iid
"""
...
# ── Main ────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="FedAvg on F3")
    ...
    exp_name = f"fedavg_f3_{split_mode}_{num_clients}c_{num_rounds}r{sr_tag}"
    save_dir = os.path.join(PROJECT_ROOT, "results", exp_name)
    os.makedirs(save_dir, exist_ok=True)

    print("=" * 70)
    print(f"FedAvg — F3 Netherlands | {split_mode.upper()} | {num_clients} clients")
    print("=" * 70)
