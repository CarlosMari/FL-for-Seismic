"""
Federated Proximal (FedProx) Simulation on Parihaka Seismic Dataset
===================================================================
Same pipeline as FedAvg but adds a proximal regularization term
mu/2 * ||w - w_global||^2 to each client's local objective, which
penalizes client drift from the global model.

Usage:
    python train_fedprox.py --num_clients 3 --split noniid --mu 0.01
    python train_fedprox.py --num_clients 20 --split noniid --mu 0.1
"""
...
def proximal_term(model, global_params):
    """Compute (mu/2) * ||w - w_global||^2 (without the mu/2 factor)."""
    prox = 0.0
...
def local_train_fedprox(model, loader, criterion, device, local_epochs, lr,
    ...
    """Local training with FedProx proximal regularization."""
    model.train()
    model.to(device)
...
# ── Main ────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="FedProx on Parihaka")
    ...
    exp_name = f"fedprox_mu{mu}_{split_mode}_{num_clients}c_{num_rounds}r"
    save_dir = os.path.join(PROJECT_ROOT, "results", exp_name)
    os.makedirs(save_dir, exist_ok=True)

    print("=" * 70)
    print(f"FedProx (mu={mu}) — Parihaka | {split_mode.upper()} | {num_clients} clients")
    print("=" * 70)
    ...
    for round_idx in range(num_rounds):
        ...
        # Store global parameters for proximal term (detached)
